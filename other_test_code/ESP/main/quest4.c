...
double fintegral, fderivative, f_error, fprev_error, foutput, fdt;

void PID_front() {

while (1) {
    int fvalue = lidarGetRange();
    f_error = fbaseline - fvalue;
    fdt = 0.1; // important!!!
    fintegral = fintegral + f_error * fdt;
    fderivative = (f_error - fprev_error) / fdt;
    foutput = Kp * f_error + Ki * fintegral + Kd * fderivative;
    fprev_error = f_error;
...
double wintegral, wderivative, werror, wprev_error, woutput, wdt;

void PID_speed() {

  while (1) {
    double wvalue = speed_app_main();
      werror = wbaseline - wvalue;
      wdt = 0.1; // important!!!
      wintegral = wintegral + werror * wdt;
      wderivative = (werror - wprev_error) / wdt;
      woutput = Kp * werror + Ki * wintegral + Kd * wderivative;
      wprev_error = werror;

      printf("Output: %f\n", woutput);
      if ((int)soutput == 0) {
        turnStraight();
      } else if ((int)woutput < 0) {
        turnLeft();
      } else if ((int)woutput > 0) {
        turnRight();
      } else {
...
static void call_PID() {
  ...
  wdt = 0.0;
  wprev_error = 0.0;		// Set up PID loop
  wintegral = 0.0;
